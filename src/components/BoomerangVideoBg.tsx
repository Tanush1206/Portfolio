...
// Each captured frame is a full canvas: at 960x540 that is ~2MB of memory, so
// an uncapped capture of a long clip runs to hundreds of megabytes. 120 frames
// is ~4s of source, which boomerangs to an ~8s cycle.
...
export default function BoomerangVideoBg({ src, className }: Props) {
  ...
  useEffect(() => {
    ...
    const captureFrame = () => {
      ...
      if (frames.length >= MAX_FRAMES) finalize();
    };
